...
{
    protected $signature = 'ai:context {--path=AGENTS.md : Project-relative file to update} {--stdout : Print the generated section without writing the file}';

    protected $description = 'Generate AI project context and insert it into AGENTS.md without touching the rest of the file.';
    ...
    public function handle()
    {
        ...
        $this->info('AI context updated in ' . $path);
    ...
    protected function buildSection()
    {
        $lines = [
            ...
            '',
            '> Automatically generated by `php artisan ai:context`. Edit other parts of `AGENTS.md`; this block is replaced when regenerated.',
            '',
            '## Agent Instructions',
            '',
            '- Before making Laravel AutoCrud, CRUD, model, migration, route, Inertia or Vue page changes, load and follow the `laravel-auto-crud` skill.',
            '- Prefer AutoCrud model metadata over manual CRUD controllers, FormRequests, resource routes or duplicated frontend logic.',
            '- After making changes that affect models, database schema, routes, Inertia pages or Vue components, run `php artisan ai:context` and keep this section updated.',
            '- Treat this generated context as the current project map for AI-assisted work.',
            '',
    ...
    protected function getAutoCrudRelations($class)
    {
        ...
        try {
            ...
            // The property may not exist or may be inaccessible in older models.
        }
    ...
    protected function renderModels($models)
    {
        ...
        if (empty($models)) {
            return array_merge($lines, ['No models found in `app/Models`.', '']);
        }
        ...
        $lines = array_merge($lines, $this->renderModelGroup('### AutoCrud Models', $autoCrud));
        $lines = array_merge($lines, $this->renderModelGroup('### Regular Models', $normal));
    ...
    protected function renderModelGroup($title, $models)
    {
        ...
        if (empty($models)) {
            return array_merge($lines, ['No models in this group.', '']);
        }

        foreach ($models as $model) {
            $lines[] = '#### ' . $model['name'];
            $lines[] = '';
            $lines[] = 'Class: `' . $model['class'] . '`';
            ...
            if (!empty($model['fields'])) {
                $lines[] = 'AutoCrud Fields:';
                foreach ($model['fields'] as $field) {
                    $lines[] = '- ' . ($field['field'] ?? 'no_field') . ' (' . ($field['type'] ?? 'no_type') . ')' . (!empty($field['name']) ? ': ' . $field['name'] : '');
                }
            }
    ...
    protected function renderDatabase($tables)
    {
        ...
        foreach ($tables as $table) {
            ...
            foreach ($table['columns'] as $column) {
                $lines[] = '| ' . ($column['name'] ?: '') . ' | ' . ($column['type'] ?: '') . ' | ' . ($column['nullable'] === null ? '' : ($column['nullable'] ? 'yes' : 'no')) . ' |';
            }
    ...
    protected function renderInertiaPages($pages)
    {
        ...
        if (empty($pages)) {
            return array_merge($lines, ['No routes rendering Inertia were detected.', '']);
        }

        foreach ($pages as $page) {
            $lines[] = '### ' . $page['component'];
            $lines[] = '';
            $lines[] = 'Route: `' . implode('|', $page['methods']) . ' ' . $page['uri'] . '`';
    ...
    protected function renderVueComponents($files)
    {
        ...
        return array_merge(
            $this->renderVueGroup('## Vue Pages', $pages),
            $this->renderVueGroup('## Vue Components', $components)
        );
    }
    ...
    protected function renderVueGroup($title, $files)
    {
        ...
        if (empty($files)) {
            return array_merge($lines, ['No Vue files found in this group.', '']);
        }
